...
size_t jobj_process_file(uint8_t *data, size_t fsize, struct ObjHeader *header, uint8_t **code, struct LinkTable *table) {
    ...
    if (header->unres_sym_len == 0 && header->res_sym_len == 0) {
        ...
        *code = malloc(fsize - sizeof(*header));
        memcpy(*code, data, fsize - sizeof(*header));
        return fsize - sizeof(*header);
    } else {
